/**
 * Reset the database to a known state: truncate every table, then insert
 * fixtures. Destructive — it reads DATABASE_URL (or the discrete DB_* vars)
 * like the app does, so pointing it at anything other than a throwaway
 * database is on the caller.
 *
 * Shared by the local CLI (`scripts/seed.js`, `npm run seed`) and the
 * migrator Lambda's `{"action":"seed"}` (migrator.js gates that one to
 * DEPLOY_ENV === 'dev' — that check is the only thing standing between this
 * and prod, since this module itself has no such guard).
 *
 * Runs in one transaction, so a failure halfway leaves the database as it
...
// Locally this defaults to Vite's dev server, matching the images actually
// served by `npm run dev` in dejavu/. A deployed environment has no such
